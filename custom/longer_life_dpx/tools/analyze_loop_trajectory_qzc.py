...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', default='P100_ent1_route1_case1', help='dataset name (default: %(default)s)')
    ...
    logging.info(f'load info odo from file: {args.dataset_name}')
    # 1.先找到dataset_name对应的目录
    [logFiles, Filenames, LogDirPaths] = traverse_by_brand(current_dir+"/log", "", args.dataset_name, 0, True) # 0代表当前目录

    logging.info(f'len(logFiles): {len(logFiles)}')
    # 2.找到dataset_name对应的csv
    if len(logFiles) == 1:
        target_dir = logFiles[0]
        logging.info(f'target_dir: {target_dir}')
        [logFiles1, Filenames1, LogDirPaths1] = traverse_by_brand(target_dir, "", ".csv", 3, False)
        if len(logFiles1) == 1:
            odo_file = logFiles1[0] # "/xxx/xxx/.../InfoOdos.csv"
            logging.info(f'odo_file: {odo_file}')

            analyzer = Analyzer(Path(odo_file))
            analyzer.run()
